...
async def get_blogs(db: Session = Depends(get_db)):
    ...
    blogs = blog_service.get_all_blogs_summary()
    return blogs
...
async def get_published_blogs(db: Session = Depends(get_db)):
    ...
    blog_service = BlogService(db)
    blogs = blog_service.get_published_blogs_summary()
    return blogs
...
async def search_blogs(q: str, db: Session = Depends(get_db)):
    """搜索博客摘要（不包含content）"""
    blog_service = BlogService(db)
    blogs = blog_service.search_blogs_summary(q)
    return blogs


@router.get("/author/{author_id}", response_model=List[BlogSummary])
async def get_blogs_by_author(author_id: str, db: Session = Depends(get_db)):
    """根据作者获取博客摘要（不包含content）"""
    blog_service = BlogService(db)
    blogs = blog_service.get_blogs_by_author_summary(author_id)
    return blogs


@router.get("/category/{category}", response_model=List[BlogSummary])
async def get_blogs_by_category(category: str, db: Session = Depends(get_db)):
    """根据分类获取博客摘要（不包含content）"""
    blog_service = BlogService(db)
    blogs = blog_service.get_blogs_by_category_summary(category)
    return blogs


# 通用路由放在最后，避免误匹配具体路由
@router.get("/{blog_id}", response_model=Blog)
async def get_blog_by_id(blog_id: str, db: Session = Depends(get_db)):
    """根据ID获取博客完整内容"""
    blog_service = BlogService(db)
...
async def create_blog(
    ...
    new_blog = blog_service.create_blog(blog_create, current_user_id)
    return new_blog
...
async def update_blog(
    ...
    success = blog_service.update_blog(blog_id, updates)
    if not success:
        raise HTTPException(status_code=404, detail="博客不存在")

    # 返回更新后的博客
    updated_blog = blog_service.get_blog_by_id(blog_id)
    return updated_blog
...
async def increment_blog_views(blog_id: str, db: Session = Depends(get_db)):
    """增加博客浏览量"""
    blog_service = BlogService(db)
    success = blog_service.increment_blog_views(blog_id)
...
async def save_blogs_batch(
    ...
    blog_service.save_blogs_batch(blogs_data)
